...
For a more visual/presentation-friendly version of this plan, see:

- [`IFRS18-Project-Timeline.png`](IFRS18-Project-Timeline.png) — a
  rendered Gantt chart (shown below), generated by
  [`generate_gantt_chart.py`](generate_gantt_chart.py).
- [`IFRS18_Adoption_Project_Plan.xlsx`](IFRS18_Adoption_Project_Plan.xlsx)
  — now opens on a **Dashboard** tab with KPI summary cards, the
  timeline image, and native Excel charts (effort by phase/role/work
  type).
- [`IFRS18-Project-Plan-Overview.pptx`](IFRS18-Project-Plan-Overview.pptx)
  — a 5-slide executive-summary deck (title & KPIs, timeline, effort
  breakdown charts, milestones, risks & next steps), generated by
  [`generate_project_plan_pptx.py`](generate_project_plan_pptx.py), for
  sharing with stakeholders who want a presentation rather than a
  spreadsheet or markdown table.

![IFRS 18 Adoption — SAP Implementation Timeline](IFRS18-Project-Timeline.png)

## Visual Timeline (Gantt, editable source)

The rendered image above is generated from the same phase dates as this
mermaid diagram (kept here as an editable text-based source and as a
fallback for viewers that don't render images):
